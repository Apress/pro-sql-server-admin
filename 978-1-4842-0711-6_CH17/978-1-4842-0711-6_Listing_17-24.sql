...
BEGIN
	...
	IF (SELECT COUNT(*) FROM sys.databases WHERE source_database_id = @DBID) > 0
		SET @SS_Seq_No = (SELECT TOP 1 CAST(SUBSTRING(name, LEN(Name), 1) AS INT) FROM sys.databases WHERE source_database_id = @DBID ORDER BY create_date DESC) + 1
	ELSE
		SET @SS_Seq_No = 1
	
	--Generate the first part of the CREATE DATABASE statement

	SET @SQLStart = 'CREATE DATABASE ' + QUOTENAME(@DBName + CAST(CAST(GETDATE() AS DATE) AS NCHAR(10)) + '_ss' + CAST(@SS_Seq_No AS NVARCHAR(4))) + ' ON ' ;
	...
	(
 		SELECT 
			'(NAME = N''' + mf.name + ''', FILENAME = N''' + SUBSTRING(mf.physical_name, 1, LEN(mf.physical_name) - 4) + CAST(@SS_Seq_No AS NVARCHAR(4)) + '.ss' + '''),' AS [data()]
	...
	--Remove the etra comma, from the end of the file list
	...
	--Check to see if the requird number of snapshots exist for the database, 
        --and if so, delete the oldest

	IF (SELECT COUNT(*) FROM sys.databases WHERE source_database_id = @DBID) > @RequiredSnapshots
	BEGIN
