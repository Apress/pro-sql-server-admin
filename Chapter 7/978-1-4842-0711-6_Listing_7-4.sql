...
FROM
	(SELECT
		...
		(SELECT TOP 1 CAST(RAND() * Number AS INT) * 10000 FROM @Numbers ORDER BY NEWID()) CreditLimit,
		(SELECT TOP 1 CAST(RAND() * Number AS INT) * 9000 FROM @Numbers ORDER BY NEWID()) Balance
	FROM @Numbers a
	CROSS JOIN @Numbers b
) a

INSERT INTO CustomersDisc
...
FROM
	(SELECT
		(SELECT CAST(DATEADD(dd,(SELECT TOP 1 Number FROM @Numbers ORDER BY NEWID()),getdate())as DATE)) OrderDate,
		...
		(SELECT TOP 1 CAST(RAND() * Number AS INT) +10 * 100 FROM @Numbers ORDER BY NEWID()) NetAmount,
		(SELECT CAST(DATEADD(dd,(SELECT TOP 1 Number - 10 FROM @Numbers ORDER BY NEWID()),getdate()) as DATE)) DeliveryDate
	FROM @Numbers a
	CROSS JOIN @Numbers b
...
CREATE NONCLUSTERED INDEX NCI_Balance ON CustomersDisc(Balance)
