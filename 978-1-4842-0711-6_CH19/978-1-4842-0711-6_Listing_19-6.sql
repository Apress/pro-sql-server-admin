...
SELECT c.procedurename, d.pagesplits
 ...
 (
	...
	(
		...
		FROM
		(
			SELECT --Query the fn_xe_file_target_read_file function, to extract the raw XML
				OBJECT_NAME CapturedEvent,
				CAST(event_data AS XML) xml_data
				FROM sys.fn_xe_file_target_read_file('C:\mssql\pagesplits*.xel', NULL , NULL, NULL) as XE ) a
		) b
