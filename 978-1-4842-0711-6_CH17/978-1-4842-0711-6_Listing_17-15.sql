...
SELECT 
	...
	WHERE counter_name IN('Average Latch Wait Time (ms)','Average Latch Wait Time base') ;
...
IF (SELECT COUNT(DISTINCT counter_value) FROM @cntr WHERE counter_name = 'Average Latch Wait Time (ms)') > 2
BEGIN
SELECT
	(
		(
